...
@section('title')
    {{ localize('Scan Content') }} {{ getSetting('title_separator') }} {{ getSetting('system_title') }}
@endsection
                    ...
                    <div class="tt-page-header">
                        <div class="d-lg-flex align-items-center justify-content-lg-between">
                            <div class="tt-page-title mb-3 mb-lg-0">
                                <h1 class="h4 mb-lg-1">{{ localize('Scan Content') }}</h1>
                                <ol class="breadcrumb breadcrumb-angle text-muted">
                                    <li class="breadcrumb-item"><a
                                            href="{{ route('writebot.dashboard') }}">{{ localize('Dashboard') }}</a>
                                    </li>

                                    <li class="breadcrumb-item">{{ localize('Scan Content') }}</li>
                                </ol>
                            </div>
                            <div class="tt-action">
                            </div>
                        ...
                        <div class="row justify-content-center">
                            <div class="col-12">
                                <div class="tt-article-generate">
                                    ...
                                    <form action="{{ route('admin.content.detector.store') }}" method="POST"
                                        class="scan-contents-form" id="scanNowForm">
                                        @csrf
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <input class="form-control" name="title" id="title"
                                                placeholder="{{ localize('Type here') }}" value="{{old('title', @$content_detector->title)}}">
                                                <x-error :name="'title'"/>
                                           
                                        </div>
                                        <div class="mb-4">

                                            <label for="text" class="form-label"><span
                                            ...
                                            <textarea class="form-control" name="text" id="input-textarea" rows="14"
                                                placeholder="{{ localize('Type here') }}">{{old('text', @$content_detector->content)}}</textarea>
                                                <x-error :name="'text'"/>
                                            <div class="d-flex justify-content-between">
                                                <p class="fs-md">{{ localize('Total word') }} : <strong
                                                        id="charac-count">{{@$content_detector->response->length ?? 0}}</strong></p>
                                        ...
                                        <button class="btn btn-primary scanNow" id="scanNow">
                                            <span class="me-2 btn-create-text">{{ localize('Scan Text') }}</span>
